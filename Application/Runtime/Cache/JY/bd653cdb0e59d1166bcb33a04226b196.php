	  	...
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
					  <td><?php echo ($row['zsxm']); ?></td>
                      <td><?php echo ($row['hm']); ?></td>
                      <td> <img src="http://<?=$_SERVER['SERVER_NAME']?>/Public/<?php echo ($row['sfz']); ?>" data-action="zoom"  width="50" height="50"></td>
                      <td> <img src="http://<?=$_SERVER['SERVER_NAME']?>/Public/<?php echo ($row['whxk']); ?>" data-action="zoom"  width="50" height="50"></td>
	...
	<script type="text/javascript">
            function  jj(id,stat){
              ...
              var s="确认拒绝";
      			...
      			if(confirm(""+s+"?")){
				$.post("<?php echo U('Mgt/whrz');?>",{id:id,stat:stat},function(data){
					if(data==1){
				    alert("操作成功")
                       location.reload();
					}else{
