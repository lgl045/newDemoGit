	  	...
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td><img src="http://<?=$_SERVER['SERVER_NAME']?>/Public/<?php echo ($row['pic']); ?>" data-action="zoom"  width="50" height="50"></td>                     
                     <!-- <td><?php echo ($row['age']); ?></td>
                      <td><?php echo ($row['sex']); ?></td>-->
                      <td>
                         <?php if( $row['sfz'] == null): ?>未认证
                         <?php else: ?>  
                          <img src="http://<?=$_SERVER['SERVER_NAME']?>/Public/<?php echo ($row['sfz']); ?>" data-action="zoom"  width="50" height="50"><?php endif; ?>
                      </td>
                       <td>
                          <?php if( $row['sfzb'] == null): ?>未认证
                         <?php else: ?>  
                          <img src="http://<?=$_SERVER['SERVER_NAME']?>/Public/<?php echo ($row['sfzb']); ?>" data-action="zoom"  width="50" height="50"><?php endif; ?>
                      </td>
                   <!--  
					  <td><?php echo ($row['wx']); ?></td>
                      <td><?php echo ($row['email']); ?></td> 
                      <td><?php echo ($row['qq']); ?></td> 
                      <th><?php echo ($row['jf']); ?></th>
					-->
					  ...
					  <!--<td>                     
                         <?php if( $row['stat'] == 0): ?>正常
                         <?php else: ?>已停用<?php endif; ?></td>  
					  -->           
                      <td>
                        <a href="/index.php/JY/Mgt/update/id/<?php echo ($row['id']); ?>">修改</a> &nbsp;
                        <a href="javascript:void(0);" class="del" id="<?php echo ($row['id']); ?>">删除</a><br/><br/>
                        <a href="/index.php/JY/Mgt/mexq/id/<?php echo ($row['id']); ?>">需求</a>   &nbsp;
                        <a href="/index.php/JY/Mgt/mejn/id/<?php echo ($row['id']); ?>">技能</a>   <br/><br/>
                        <a href="/index.php/JY/Mgt/mexj/id/<?php echo ($row['id']); ?>">下级</a>   &nbsp;
                        <a href="/index.php/JY/Mgt/mexq/id/<?php echo ($row['id']); ?>">相亲</a>
                     </td>  
