		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
						 <td><?php echo ($row['id']); ?></td>
				 	     <td><img src="http://<?=$_SERVER['SERVER_NAME']?>/Public/<?php echo ($row['pic']); ?>" data-action="zoom"  width="60" height="60"></td>
                         <td><?php echo ($row['addtime']); ?></td>
                      	 <td>                     
                         	<?php if( $row['stat'] == 0): ?>首页
                         	<?php elseif( $row['stat'] == 1): ?>推荐
                            <?php elseif( $row['stat'] == 2): ?>娱乐
                            <?php elseif( $row['stat'] == 4): ?>订单
                            <?php elseif( $row['stat'] == 3): ?>相亲
                            <?php elseif( $row['stat'] == 5): ?>技能
                            <?php elseif( $row['stat'] == 6): ?>商城
                            <?php elseif( $row['stat'] == 7): ?>商学院<?php endif; ?>
                         </td>  
						 <td><a href="/index.php/JY/Mgt/elb/id/<?php echo ($row['id']); ?>">修改</a>|<a href="javascript:void(0);" class="del" id="<?php echo ($row['id']); ?>">删除</a></td>
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	<script type="text/javascript">
        ...
        $(this).text($(this).text() + "...");  
			...
			var ob = $(this);
			if(confirm('确定删除此用户？')){
				$.post("<?php echo U('Mgt/dellb');?>",{id:id},function(data){
					if(data==1){
