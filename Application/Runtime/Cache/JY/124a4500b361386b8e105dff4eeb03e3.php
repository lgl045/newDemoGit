	  	...
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td><?php echo ($row['jstime']); ?></td>
                      <td><?php echo ($row['kswz']); ?></td>
                      <td><?php echo ($row['jswz']); ?></td>
              		  <td><?php echo ($row['type']); ?></td>
                      <td><?php echo ($row['money']); ?></td>                      
					  <td>                     
                         <?php if( $row['stat'] == 0): ?>未接
                         <?php else: ?> 已接<?php endif; ?></td>              
                      <td><a href="/index.php/JY/Mgt/xgdd/id/<?php echo ($row['id']); ?>">修改</a>|<a href="javascript:void(0);" class="del" id="<?php echo ($row['id']); ?>">删除</a></td>  
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	<script type="text/javascript">
		$(".del").click(function(){
			...
			var ob = $(this);
			if(confirm('是否确定删除？')){
				$.post("<?php echo U('Mgt/del_dd');?>",{id:id},function(data){
					if(data==1){
