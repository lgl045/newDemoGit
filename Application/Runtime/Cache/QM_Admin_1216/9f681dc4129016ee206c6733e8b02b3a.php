	...
	<div class="layui-form news_list">
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
						<td><?php echo ($row['id']); ?></td>
						<td><?php echo ($row['username']); ?></td>
						<td><?php echo ($row['phone']); ?></td>
						<td><img src="http://115.159.114.168/<?php echo ($row['pic']); ?>" data-action="zoom"  width="80" height="80"></td>
						<td><?php echo (date("Y-m-d h:m:s",$row['addtime'])); ?></td>
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	<script type="text/javascript">
		$(".del").click(function(){
			...
			var ob = $(this);
			if(confirm('确定删除此用户？')){
				$.get("<?php echo U('Mgt/del');?>",{id:id},function(data){
					if(data==1){
