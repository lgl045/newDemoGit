...
-->
          ...
          <a class="layui-btn search_btn" href="/index.php/QM_Admin_1216/Mgt/addfl">添加</a>
		...
		<!--     <colgroup>
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($row['id']); ?>" >
						<td class="as"><?php echo ($row['fname']); ?></td>
                      	<td><?php echo ($row['addtime']); ?></td>
						<td><a href="javascript:void(0);" onclick="del(<?php echo ($row['id']); ?>)">删除</a></td>
		...
		function del(id){
			...
			if(confirm('确定删除？')){
				...
				if(!id){
					...
					$.post('/index.php/QM_Admin_1216/Mgt/selfl',{id:id},function(data){
						if(data == 1){
							$("#"+id).remove();
						}else{
							alert('服务器错误！');
