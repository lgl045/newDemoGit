		...
		<!--     <colgroup>
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($goods)): $i = 0; $__LIST__ = $goods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php echo ($row['danjia2']); ?></td>
						<td><img src="http://115.159.114.168/<?php echo ($row['img']); ?>" data-action="zoom"  width="80" height="80"></td>
						
						<td><?php echo (date("Y-m-d h:m:s",$row['addtime'])); ?></td>
						<td><a href="javascript:void(0);" class="del" id="<?php echo ($row['user_id']); ?>">删除</a></td>
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	<script type="text/javascript">
		$(".del").click(function(){
			...
			if(confirm('确定删除此用户？')){
				$.get("<?php echo U('Mgt/shangdel');?>",{id:id},function(data){
					if(data==1){
