...
<body class="childrenBody">
 <blockquote class="layui-elem-quote news_search">
		<div class="layui-inline">
		    ...
		    <a class="layui-btn search_btn"   href="/index.php/QM_Admin_1216/Mgt/add_jb">添加</a>
		</div>
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($res)): $i = 0; $__LIST__ = $res;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td><?php echo ($row['num']); ?></td>
						...
						<td><?php if($row['type'] == 0 ): ?>扫雷场
   						 <?php else: ?> 接龙场场<?php endif; ?></td>
					
						
					
						<td><a href="javascript:void(0);" class="del" id="<?php echo ($row['jid']); ?>">删除</a></td>
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	<script type="text/javascript">
		$(".del").click(function(){
			...
			if(confirm('确定删除此用户？')){
				$.post("<?php echo U('Mgt/del_jb');?>",{id:id},function(data){
					if(data==1){
