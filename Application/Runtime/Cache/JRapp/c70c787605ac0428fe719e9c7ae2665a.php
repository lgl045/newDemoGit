		    	...
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                      ...
                      <td><?php if($row['code'] == 1 ): ?><font color="red"> 已退票</font>
                       ...
                       <td><?php echo ($row['ysk']); ?></td>
                          <td>
                          <?php if($row['sh'] == 0): ?>未审核
                          <?php elseif($row['sh'] == 1): ?> <font color="green"> 审核成功</font>
                          <?php else: ?> <font color="red"> 审核未通过</font><?php endif; ?>
                      	  </td>
                                              
                    	  <td><?php if($row['sh'] == 0): ?><a href="javascript:void();" class="sh" id="<?php echo ($row['id']); ?>">审核</a>
                          <?php elseif($row['sh'] == 1): ?> <a href="javascript:void();" class="shn" id="<?php echo ($row['id']); ?>">取消审核</a>
                          <?php else: ?> <a href="javascript:void();" class="sh" id="<?php echo ($row['id']); ?>">审核</a><?php endif; ?></td>
