...
<body class="childrenBody">
	  	...
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($row['id']); ?>">
					...
					<td><?php echo ($row['qkr']); ?></td>
					<td><?php echo ($row['yj']); ?></td>
                    <td><?php echo ($row['zw']); ?></td>
				    <td><?php echo ($row['yqsj']); ?></td>
                    <td><?php if($row['qdzt'] == 0 ): ?>待接
                    <?php elseif($row['qdzt'] == 1 ): ?> 
                       待完成
                     <?php else: ?> 
                     待接<?php endif; ?>
                        </td>
               	    <td><?php echo ($row['addtime']); ?></td>
                              ...
                              <td><?php if($row['shzt'] == 0 ): ?><a href="javascript:void(0);" class="del" id="<?php echo ($row['id']); ?>">通过审核</a>
                            <?php else: ?> 
				      ...
				      <td><a href="/index.php/JRapp/Mgt/xqbl/id/<?php echo ($row['id']); ?>" >详情</a></td>
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	$(".del").click(function(){
			...
			var ob = $(this);
			if(confirm('确定通过审核？')){
				$.post("<?php echo U('Mgt/blzcs');?>",{id:id},function(data){
					if(data==1){
      	...
      	$(".bh").click(function(){
			...
			var ob = $(this);
			if(confirm('确定驳回？')){
				$.post("<?php echo U('Mgt/blzcd');?>",{id:id},function(data){
					if(data==1){
                       alert("已驳回！");
					window.location.reload();
                    }else{
