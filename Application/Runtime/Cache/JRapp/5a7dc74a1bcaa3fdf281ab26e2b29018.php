	  	...
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($row['id']); ?>">
					...
					<td><?php echo ($row['bxlc']); ?></td>
                    <td><?php echo ($row['spsj']); ?></td>
				    <td><?php echo ($row['color']); ?></td>
                    <td><?php echo ($row['money']); ?></td>
					<td><?php echo ($row['sfmoney']); ?></td>
                      
                      
                          <td><?php if($row['sh'] == 0 ): ?><a href="javascript:void(0);" class="del" id="<?php echo ($row['id']); ?>">通过审核</a>
                            <?php else: ?> 
                            <a href="javascript:void(0);"><font color="green">已过审核</font></a><?php endif; ?>
                        </td>
                        <td>
                          <?php if($row['sh'] == 1 ): ?><a href="javascript:void(0);" ><font color="grey">驳回</font></a>
                            <?php else: ?> 
                           <a href="javascript:void(0);" class="bh" id="<?php echo ($row['id']); ?>">驳回</a><?php endif; ?>
                          
                          </td>
                      
				      <td><a href="/index.php/JRapp/Mgt/xqqczy/id/<?php echo ($row['id']); ?>" >详情</a></td>
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	<script type="text/javascript">
		$(".del").click(function(){
			...
			var ob = $(this);
			if(confirm('确定通过审核？')){
				$.post("<?php echo U('Mgt/qcsh');?>",{id:id},function(data){
					if(data==1){
