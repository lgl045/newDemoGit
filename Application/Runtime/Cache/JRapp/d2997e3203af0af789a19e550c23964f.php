...
<body class="childrenBody">
	  	...
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
				...
				<td><input type="number" id="pxs_<?php echo ($row['id']); ?>" value="<?php echo ($row['px']); ?>" onBlur="px(<?php echo ($row['id']); ?>)" style="width:50%; "/></td>
                       ...
                       <td><img src="<?php echo ($row['pic1']); ?>" data-action="zoom" width="60" height="60" style="cursor: -webkit-zoom-in;"></td>
                          <td><img src="<?php echo ($row['pic2']); ?>" data-action="zoom" width="60" height="60" style="cursor: -webkit-zoom-in;"></td>
                          <td><img src="<?php echo ($row['pic3']); ?>" data-action="zoom" width="60" height="60" style="cursor: -webkit-zoom-in;"></td>
                      <td><?php echo ($row['ed']); ?></td>
                        <td><?php echo ($row['jj']); ?></td>
                      
          
                       <td><?php echo ($row['addtime']); ?></td>
    						<td><a href="/index.php/JRapp/Mgt/edtzq/id/<?php echo ($row['id']); ?>">修改</a>|<a href="javascript:void(0);" class="del" id="<?php echo ($row['id']); ?>">删除</a></td>
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
    ...
    $.post("<?php echo U('Mgt/pxdt');?>",{num:num,ids:ids},function(data){
					if(data==1){
				         alert("操作成功");
					}else if(data==3){
                    alert("输入值重复");
		...
		$(".del").click(function(){
			...
			var ob = $(this);
			if(confirm('确定删除此用户？')){
				$.post("<?php echo U('Mgt/deldt');?>",{id:id},function(data){
					if(data==1){
