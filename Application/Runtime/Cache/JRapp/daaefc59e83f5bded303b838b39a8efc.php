...
<body class="childrenBody">
	  	...
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td><?php echo ($row['zfbh']); ?></td>
                        <td><?php echo ($row['addtime']); ?></td>
                        <td><?php echo ($row['money']); ?></td>
               
                      <td>   <?php if($row['stat'] == 0 ): ?>未审批
                        <?php else: ?> 
                        <font color="green">已审批提现</font><?php endif; ?>
                        
                      </td>  
                      
                      
                   	<td>  
                      <?php if($row['stat'] == 1 ): ?><a href="javascript:void(0);" >确认</a>
                        <?php else: ?> 
                        
                      <a href="javascript:void(0);" class="del" onclick="del(<?php echo ($row['id']); ?>,<?php echo ($row['uid']); ?>,<?php echo ($row['money']); ?>)">确认</a><?php endif; ?></td> 
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	function del(id,axfid,money){
			...
			if(confirm('确定审批？')){
				$.post("<?php echo U('Mgt/txqqsp');?>",{id:id,uid:axfid,money:money},function(data){
					if(data==1){
                            alert("操作成功！")
