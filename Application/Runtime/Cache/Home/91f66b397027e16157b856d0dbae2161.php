    ...
    swal({
     ...
     <div class="col-lg-10">
        <div class="ucBox ucMinHeight">
          <div class="ucCrumbs">
            <span><a href="/index.php/home/Member/index">我的化学加</a></span>/
            <span>简历管理</span>/
            <span>我的简历</span>
          </div>
          <h1 class="uc_Tit pr">我的简历<button class="ucBtn pa abs-r15t" data-toggle="modal" data-target="#creatResumeModal">创建新简历</button></h1>
          <!-- 创建简历弹框 S-->
          <div class="modal fade sendResModel" id="creatResumeModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                <div class="modal-header">
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                  <h4 class="modal-title f14" id="myModalLabel">创建新简历</h4>
                </div>
                <div class="modal-body">
                  <form>
                    <div class="form-group">
                      <label for="resumeTit f16">请填写创建的简历名称：</label>
                      <input type="text" class="form-control" id="resumeTit" placeholder="填写简历名称，字数不超过10个字" maxlength="10">
                    </div>
                    <div class="tr">
                      <button type="button" class="btn btn-default" data-dismiss="modal">取消</button>
                      <button class="btn btn-primary" type='button' onclick="sub();" />确定</button>
                    </div>
                  </form>
                </div>
              </div>
...
$(".pubsec").each(function(){
  var self = $(this);
  var cek;
  if(self.find("input").attr("checked")){
   self.find(".togText").text("公开");
   cek = 0;
  }else{
    self.find(".togText").text("保密");
    cek = 1;
  }
  self.find(".tcb").click(function(){
    if(cek == 1){
      self.find(".togText").text("公开");
      cek = 0;
    }else{
    ...
    function del(id){
      swal({
        ...
        animation: "slide-from-top"
          ...
          $.ajax({
            url:'/index.php/home/member/delCv',
            type:'post',
            data:'id='+id,
            datatype:'json',
    ...
    function change(id){
        $.ajax({
          url:'/index.php/home/member/cvNameEdit',
          type:'post',
          data:'id='+id+'&s=y',
          datatype:'json',
    ...
    function sub(){
      var name = $('#resumeTit').val();

      $.ajax({
          url:'/index.php/home/member/createCv',
          type:'post',
          data:'name='+name,
          datatype:'json',
