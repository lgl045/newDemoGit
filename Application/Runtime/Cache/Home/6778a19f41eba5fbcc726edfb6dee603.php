...
<ul class="myaccount-menu">
        ...
        <li class="">
            ...
            <a class="active" href="/index.php/Home/accoun/myorder" title="我的店铺">
    ...
    <li class="">
     ...
     <li class="">
            ...
            <a class="edit-address" href="/index.php/Home/accoun/goods_gl" title="商品管理">
                                    <i class="fa fa-truck"></i>
                                <span>商品管理</span>
            </a>
	...
	<table class="shop_table shop_table_responsive my_account_orders">
		...
		<thead>
			<tr>
									...
									<th class="order-total"><span class="nobr">合计</span></th>
									<th class="order-actions"><span class="nobr">&nbsp;</span></th>
							</tr>
		</thead>

		<tbody>
          <?php if(is_array($order)): foreach($order as $key=>$order): ?><tr class="order">
											<td class="order-number" data-title="订单号">
								             		<?php echo ($order['danhao']); ?>			
													</td>
											<td class="order-date" data-title="日期">
															<time datetime="2018-03-19T00:21:07+00:00"><?php echo ($order['time']); ?></time>

													</td>
											<td class="order-status" data-title="状态">
                                              
													<?php if( $order['stat'] == 1): ?>待付款
                                                    <?php elseif($order['stat'] == 2 ): ?>
                                                      待发货
                                                    <?php elseif($order['stat'] == 3 ): ?>
                                                      发货中
                                                      <?php elseif($order['stat'] == 4 ): ?>
                                                      待收货
                                                      <?php elseif($order['stat'] == 5 ): ?>
                                                      待评价
                                                      <?php elseif($order['stat'] == 6 ): ?>
                                                      订单已完成
                                                      <?php elseif($order['stat'] == 7 ): ?>
                                                      订单申请退款
                                                             <?php elseif($order['stat'] == 200 ): ?>
                                                      待付款
                                                      <?php else: ?>
                                                      取消订单<?php endif; ?>
											</td>
											<td class="order-total" data-title="合计">
															<span class="woocommerce-Price-amount amount"><span class="woocommerce-Price-currencySymbol">¥</span><?php echo ($order['zongjia']); ?></span> 为 <?php echo ($order['num']); ?> 件物品
													</td>
											<td class="order-actions" data-title="&nbsp;">
                                              
                                              <!--onclick="savestat(<?php echo ($order['id']); ?>)"          javascript:void(0)-->
                                              <?php if( $order['stat'] == 2): ?><a href="  javascript:void(0)"  onclick="savestat(<?php echo ($order['id']); ?>)" class="button pay">确认收货</a>
                                                                    <?php elseif($order['stat'] == 1 ): ?>
                                                            <a href="/index.php/Home/accoun/zf/id/<?php echo ($order['id']); ?>"  class="button pay">确认付款</a>
                                                    <a href="javascript:void(0)" onclick="del(<?php echo ($order['id']); ?>)" class="button cancel">确认取消</a>	
                                                 <?php elseif($order['stat'] == 4): ?>
                                                     		<a href="  javascript:void(0)"  onclick="savestat(<?php echo ($order['id']); ?>)" class="button pay">确认收货</a>
                                                 <?php elseif($order['stat'] == 200 ): ?>
                                                            <a href="/index.php/Home/accoun/zf/id/<?php echo ($order['id']); ?>"  class="button pay">确认付款</a>
                        
                                                           <?php else: ?>
                                                  <a href="  javascript:void(0)"  class="button pay">订单已完成</a><?php endif; ?>
                                          											</td>
...
<script type="text/javascript">
          function savestat(gid){

            $.post('/index.php/Home/accoun/savestats',{id:gid},function(data){
              //alert(data)
              if(data==1){
                 alert("操作成功！");
                    location.reload();
      ...
      if(confirm('确认取消该订单？')){
        $.post("<?php echo U('accoun/del');?>",{id:gid},function(data){
          if(data==1){
              alert("已取消！");
