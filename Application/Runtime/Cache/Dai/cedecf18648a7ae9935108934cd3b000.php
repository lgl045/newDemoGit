	  	...
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($user)): $i = 0; $__LIST__ = $user;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr id="<?php echo ($row['id']); ?>">
				    ...
				    <td><?php echo (substr($row['xphone'],0,3)); ?>****<?php echo (substr($row['xphone'],8,3)); ?></td>
                    <td><?php echo ($row['ed']); ?></td>
					<td><?php echo ($row['addtime']); ?></td>
                      
                      
                      
				      <td><a href="/index.php/Dai/Mgt/xqdata/id/<?php echo ($row['id']); ?>" >详情</a><br><br><br><br><a href="javascript:void(0);" class="del" id="<?php echo ($row['id']); ?>">删除</a></td>
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	<script type="text/javascript">
		$(".del").click(function(){
			...
			var ob = $(this);
			if(confirm('确定删除此用户？')){
				$.post("<?php echo U('Mgt/delz');?>",{id:id},function(data){
					if(data==1){
