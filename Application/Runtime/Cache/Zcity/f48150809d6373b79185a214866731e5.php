	  	...
	  	<table class="layui-table">
		    ...
		    <tbody class="users_content">
		    	<?php if(is_array($data)): $i = 0; $__LIST__ = $data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$row): $mod = ($i % 2 );++$i;?><tr>
						...
						<td><?php if($row['pic'] == '/Public/img/mr.jpg' ): ?><img src="http://dai.juhuaian.cn/Public/img/mr.jpg" data-action="zoom"  width="60" height="60">
                        <?php else: ?> 
                    <img src="http://dai.juhuaian.cn/Public/<?php echo (substr($row['pic'],2,-2)); ?>" data-action="zoom"  width="60" height="60"><?php endif; ?></td>
<td><?php echo ($row['phone']); ?></td>
<td><?php echo ($row['addtime']); ?></td>
						<td><a href="/index.php/Zcity/Mgt/xqzjzy/id/<?php echo ($row['id']); ?>">详情</a>|<a href="javascript:void(0);" class="del" id="<?php echo ($row['id']); ?>">删除</a></td>
  ...
  <script type="text/javascript" src="/Public/js/zooming.js"></script>
	...
	<script type="text/javascript">
		$(".del").click(function(){
			...
			var ob = $(this);
			if(confirm('确定删除？')){
				$.post("<?php echo U('Mgt/delzj');?>",{id:id},function(data){
					if(data==1){
